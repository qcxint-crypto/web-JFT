...
import { allKanjiN3 } from './data'

const FIELDS = ['kanji', 'reading', 'meaning'] as const
type Field = typeof FIELDS[number]
type KanjiEntry = (typeof allKanjiN3)[number]
...
const clampQuestionCount = (value: number) => {
  ...
  return Math.max(5, Math.min(allKanjiN3.length, Math.round(value)))
}
...
export default function KanjiN3QuizPage() {
  ...
  const generateQuiz = (count = questionsPerSession) => {
    const selectedEntries = pickRandom(allKanjiN3, clampQuestionCount(count))
    ...
    const nextQuestions = selectedEntries.map((entry) => {
      ...
      const candidateEntries = allKanjiN3
      ...
      if (wrongOptions.length < 3) {
        const fallbackEntries = pickRandom(
          allKanjiN3.filter((item) => item.id !== entry.id && !seenValues.has(String(item[aField]))),
  ...
  const startQuiz = () => {
    ...
    setQuizInstanceKey(`kanji-n3_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`)
    setQuizStarted(true)
    setQuizEnded(false)
    setCurrentIndex(0)
    setScore(0)
  ...
  const handleGoHome = (force = false) => {
    if (
      ...
      !quizStarted ||
      quizEnded ||
      window.confirm('Kembali ke menu utama? Progress sesi Kanji N3 yang sedang berjalan akan hilang.')
    ) {
      router.push('/')
    }
  }
  ...
  useEffect(() => {
    ...
    const saveResult = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to save Kanji N3 leaderboard result.')
        }
        ...
        console.error('Failed to save Kanji N3 result:', error)
        setSaveState('error')
      }
    }
          ...
          <div className="relative space-y-6">
            ...
            <div>
              <h1 className="font-display text-4xl font-bold leading-[0.95] tracking-[-0.06em] text-slate-950 sm:text-5xl">
                Latihan Kanji N3 yang tetap informatif bahkan saat jawaban Anda salah.
              </h1>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-[26px] border border-slate-900/10 bg-white/88 p-6">
                ...
                <div className="font-display mt-3 text-4xl font-bold tracking-[-0.06em] text-slate-950">{allKanjiN3.length}</div>
                <p className="mt-2 text-sm leading-6 text-slate-600">
                  Tersedia kosakata Kanji N3 siap acak untuk drill cepat atau sesi panjang.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="glass-panel rounded-[38px] px-5 py-6 sm:px-8 sm:py-10">
          ...
          <div className="mt-6 rounded-[28px] border border-slate-900/10 bg-[color:var(--surface-strong)] p-4 sm:p-5">
            ...
            <p className="mt-3 text-center text-sm text-slate-500">
              Minimal 5 soal, maksimal {allKanjiN3.length} soal.
            </p>
          </div>
      ...
      <div className="mx-auto max-w-4xl">
        <div className="glass-panel overflow-hidden rounded-[40px] px-6 py-8 sm:px-8 sm:py-10">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="text-[11px] font-black uppercase tracking-[0.28em] text-slate-500">Kanji N3 Session Complete</div>
          ...
          <div className="mt-6 rounded-[24px] border border-slate-900/10 bg-white/90 px-5 py-4 text-sm text-slate-600">
            {saveState === 'saving' && 'Menyimpan skor ke weekly leaderboard Kanji N3...'}
            {saveState === 'saved' &&
              (leaderboardRank
                ? `Skor tersimpan di leaderboard Kanji N3. Posisi Anda saat ini: #${leaderboardRank}.`
                : 'Skor tersimpan di leaderboard Kanji N3 minggu ini.')}
            {saveState === 'error' && 'Batch selesai, tetapi leaderboard Kanji N3 belum berhasil diperbarui.'}
    ...
    <div className="space-y-6 pb-24">
      <section className="glass-panel relative overflow-hidden rounded-[38px] px-6 py-7 sm:px-8">
        ...
        <div className="relative grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
          <div>
            <div className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-500">Kanji N3 Focus Mode</div>
          </div>
      ...
      <div className="glass-panel rounded-[30px] p-5 sm:p-6">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.28em] text-slate-500">Kanji N3 Flow</p>
